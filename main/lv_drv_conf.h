 ...
 *  Common
 *------------*/
extern void lcd_dc_pin(uint8_t data);
extern void lcd_rst_pin(uint8_t data);
#define LV_DRV_DISP_INCLUDE         <stdint.h>			/*Dummy include by default*/
#define LV_DRV_DISP_CMD_DATA(val)   lcd_dc_pin(val)		/*Set the command/data pin to 'val'*/
#define LV_DRV_DISP_RST(val)        lcd_rst_pin(val)	/*Set the reset pin to 'val'*/

/*---------
 *  SPI
 *---------*/
extern void lcd_send_byte(uint8_t data);
#define LV_DRV_DISP_SPI_CS(val)									/*Set the SPI's Chip select to 'val'*/
#define LV_DRV_DISP_SPI_WR_BYTE(data)    lcd_send_byte(data)	/*Write a byte the SPI bus*/
#define LV_DRV_DISP_SPI_WR_ARRAY(adr, n)						/*Write 'n' bytes to SPI bus from 'adr'*/
